			...
			if ( $this->_options ) {
				...
				if ( isset( $tabs ) ) {
					...
					foreach ( $tabs as $tab ) {
						...
						if ( isset( ${'tab_'.$tab['id']} ) ) {
							foreach ( ${'tab_'.$tab['id']} as $tab_content ) {
								$option = new WoobizzHeaderOptionsGenerator( $tab_content, $this->_prefix );
								$div_contents .= $option;
							}
						} else {
					...
					foreach ( ${'tab_0'} as $tab_content ) {
						$option = new WoobizzHeaderOptionsGenerator( $tab_content );
						$div_contents .= $option;
					}
...
if ( ! class_exists( 'WoobizzHeaderOptionsGenerator' ) ) {
	class WoobizzHeaderOptionsGenerator {
